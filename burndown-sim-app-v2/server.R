...
shinyServer(function(input, output) {
    ...
    sim_all <- eventReactive(input$runsim, {
         ...
         for(s in 1:input$nsims){
             sim <- s
             bal_start <- round(input$sbal,0)
             cat(input$sbal,"\n")
             bal_start_set <- bal_start
             bal_start_no_draw <- bal_start
             sim_sched <- data.frame()
             for(y in 1:input$nyrs){
                 rrate <- round(min(rnorm(n=1, mean=input$arr/100, sd=input$sdrr/100), input$maxrr/100), 3)
                 ...
                 draw <- min(round(draw_ni*(1+(input$inflr/100))^y,0),bal_start+return)
    ...
    output$burndownPlot <- renderPlotly({
        psim <- sim_all() %>% ggplot(aes(x=year, y=balance, color=sim,
                                         text=paste0("bal.:",balance,
                                                     "<br>rtn:",rrate*100,"%",
                                                     "<br>draw:",draw_ni), 
                                         group=1))+
          geom_line()+
